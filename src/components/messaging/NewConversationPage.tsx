...
import { createEncryptedConversation } from '../../lib/encryptionUtils';
...
export function NewConversationPage() {
  ...
  const { prepareNewConversation, isUnlocked } = useEncryption();
      ...
      try {
        ...
        if (householdIds.length > 0) {
          ...
          const { data: professionals } = await supabase
            ...
            .eq('status', 'approved')
            .neq('professional_profile_id', profile.id);
  ...
  const toggleUser = (profileId: string) => {
    setSelectedUsers((prev) => {
      ...
      if (newSet.has(profileId)) {
        ...
        if (conversationType === 'direct' && newSet.size >= 1) {
          newSet.clear();
        }
  ...
  const handleCreate = async () => {
    if (!isUnlocked) {
      setError('Please unlock your encryption keys first');
      return;
    }
    ...
    if (conversationType === 'direct' && selectedUsers.size !== 1) {
      setError('Direct conversations must have exactly one other participant');
      return;
    }
    ...
    try {
      ...
      const participantIds = [profile.id, ...Array.from(selectedUsers)];

      const { conversationKey, encryptedKeys } = await prepareNewConversation(
        participantIds
      );

      const result = await createEncryptedConversation(
        conversationType,
        participantIds,
        encryptedKeys
      );
  ...
  const canCreate =
    ...
    (conversationType === 'group' || selectedUsers.size === 1);
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      <div className="p-4 border-b border-slate-200 bg-white">
        <div className="flex items-center gap-4 mb-4">
          ...
          <h1 className="text-xl font-bold text-slate-900">New Conversation</h1>
        </div>
          ...
          <div className="flex flex-col items-center justify-center h-full gap-4 text-slate-500 p-4">
            ...
            <p className="text-sm text-center">
              {searchQuery
                ...
                : 'Join a household to start messaging'}
            </p>
          </div>
        ) : (
      ...
      <div className="p-4 border-t border-slate-200 bg-white">
        <button
          ...
          disabled={!canCreate || creating || !isUnlocked}
        ...
        >
          {creating ? (
            ...
            `Create ${conversationType === 'direct' ? 'Direct' : 'Group'} Conversation`
          )}
        </button>
      </div>
    </div>
