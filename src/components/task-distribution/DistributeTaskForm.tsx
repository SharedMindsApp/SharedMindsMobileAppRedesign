/**
 * Distribute Task Form
 * 
 * Phase 4.3: Action component for distributing tasks to groups
 * 
 * Responsibilities:
 * - Uses mutation hook (useDistributeTask)
 * - Handles loading + error states
 * - Select group
 * - Submit → distribute task to group
 */

import { useState } from 'react';
import { useDistributeTask } from '../../hooks/distribution/useDistributeTask';
import { useGroups } from '../../hooks/groups/useGroups';

type DistributeTaskFormProps = {
  taskId: string;
  teamId: string;
  onDistributed?: () => void;
};

export function DistributeTaskForm({ taskId, teamId, onDistributed }: DistributeTaskFormProps) {
  const { distributeTask, loading, error } = useDistributeTask();
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      >
        {loading ? 'Distributing...' : 'Distribute Task'}
      </button>
    </form>
  );
}
