...
interface AlignmentBlockEnhancedProps {
  block: AlignmentBlockWithMicrotasks;
  onUpdate: () => void;
}

export function AlignmentBlockEnhanced({ block, onUpdate }: AlignmentBlockEnhancedProps) {
  ...
  const [completingMicrotaskId, setCompletingMicrotaskId] = useState<string | null>(null);
  ...
  const handleToggleMicrotask = async (microtaskId: string, currentState: boolean) => {
    if (!currentState) {
      setCompletingMicrotaskId(microtaskId);
      setTimeout(() => setCompletingMicrotaskId(null), 600);
    }
  ...
  return (
    <div className="bg-white border-2 border-gray-200 rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
      <div className="p-3">
          ...
          <div className="mt-3 space-y-2">
            <div className="space-y-1.5">
              {block.microtasks.map((microtask) => {
                const isCompleting = completingMicrotaskId === microtask.id;
                      ...
                      {microtask.is_completed && (
                        <svg
                          className={`w-3.5 h-3.5 text-white ${
                            isCompleting ? 'animate-[scale_0.3s_ease-out]' : ''
                          }`}
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
              ...
              <button
                ...
                className="p-2 text-blue-600 hover:text-blue-700 disabled:text-gray-300 disabled:cursor-not-allowed rounded-lg hover:bg-blue-50 transition-all"
                title="Add step"
              >
