 ...
 *   <WorkCreditsEditor />                — own-profile add/edit/delete (settings)
 *
 * Slice 1: self-listed only. Collaborator tagging + verification come later.
 */

import { useEffect, useState } from 'react';
import { Briefcase, Plus, Trash2, Pencil, ExternalLink, Loader2, Check, X } from 'lucide-react';
...
import {
  ...
  type WorkCredit, type WorkCreditInput,
} from '../../services/WorkCreditService';

// ── Display ─────────────────────────────────────────────────────────────────

function CreditRow({ credit }: { credit: WorkCredit }) {
  return (
...
export function WorkCreditsEditor() {
  ...
  async function load() {
    ...
    const c = await fetchWorkCredits(user.id);
    setCredits(c);
    setLoading(false);
  }
  useEffect(() => { void load(); }, []);
    ...
    <div className="space-y-3">
      ...
      {credits.map((c) => (
        editing === c.id ? (
          ...
          <div key={c.id} className="flex items-start gap-2 rounded-xl bg-surface-container-low px-3 py-2.5">
            <div className="flex-1 min-w-0">
