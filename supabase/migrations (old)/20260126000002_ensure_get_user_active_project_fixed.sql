/*
  # Ensure get_user_active_project RPC Function is Fixed
  
  Ensures the RPC function uses the correct column name `has_completed_wizard`
  instead of the non-existent `wizard_completed` column.
  
  This migration explicitly drops and recreates the function to ensure
  any old version with the wrong column reference is removed.
*/

-- Drop the function if it exists (ensures clean recreation)
DROP FUNCTION IF EXISTS get_user_active_project(uuid);
...
COMMENT ON FUNCTION get_user_active_project(uuid) IS 'Returns the user''s active project with full project data. Uses has_completed_wizard column (not wizard_completed).';
