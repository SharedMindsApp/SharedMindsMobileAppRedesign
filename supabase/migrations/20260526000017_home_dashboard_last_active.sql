-- Extend get_home_dashboard with last_active_at — the timestamp of the
-- user's most recent completed session. Powers the Quick Restart card's
-- "you've been gone N days" detection without exposing N to the user.
--
-- We deliberately don't expose the day-count number anywhere visible.
-- It's a private trigger for surfacing the warm "welcome back" card —
-- per the no-gap-no-shame product principle.
...
begin
  ...
  select
    ...
    max(coalesce(ended_at, end_time))
  into v_total_sessions, v_finished_count, v_day_set, v_last_active
  ...
  -- Upcoming scheduled (next 28 days, capped at 50).
